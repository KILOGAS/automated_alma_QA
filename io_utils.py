...
def load_summary_table(summary_path):
    ...
    df = (
        ...
        else pd.read_excel(summary_path)
    )
    ...
    for key, pattern in patterns.items():
        ...
        if isinstance(pattern, list):
            ...
            for pat in pattern:
                ...
                # For unmaskedcube, try cube_root first if available
...
def find_error_map_paths(files):
    ...
    sigma_mol_err = (
        files["sigma_mol"].replace(".fits", "_err.fits")
        if files.get("sigma_mol")
        else None
    )
    mmol_err = (
        files["mmol"].replace(".fits", "_err.fits") if files.get("mmol") else None
    )
    return {
        "ico_err": ico_err,
        "lco_err": lco_err,
        "sigma_mol_err": sigma_mol_err,
        "mmol_err": mmol_err,
    }
