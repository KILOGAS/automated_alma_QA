...
def measure_rms_cube_ends(cube_path, edge_channels=3):
    ...
    with fits.open(cube_path) as hdul:
        ...
        return {
            "rms_start": float(rms_start),
            "rms_end": float(rms_end),
...
def mask_nonblank_stats(mask_path):
    ...
    with fits.open(mask_path) as hdul:
        ...
        return {
            "mask_nonblank": int(nonblank),
            "mask_total": int(total),
            ...
            if moment0.shape == snr.shape:
                ...
                if "err" in locals() and err.shape == snr.shape:
                    ...
                    if np.any(valid):
                        ...
                        result["flag_snr_mismatch"] = (
                            frac_mismatch > 0.01
                        )  # flag if >1% mismatch
                        if result["flag_snr_mismatch"]:
                            result["snr_mismatch_summary"] = (
                                f"{frac_mismatch:.2%} of pixels differ by >1%"
                            )
                    else:
                        result["snr_mismatch_frac"] = None
                        result["flag_snr_mismatch"] = True
                        result["snr_mismatch_summary"] = (
                            "No valid pixels for SNR comparison"
                        )
            else:
                result["spatial_shape_mismatch"] = (
                    f"moment0 shape {moment0.shape} != snr shape {snr.shape}"
                )
    except Exception as e:
...
def compare_sigma_mol_to_ico(sigma_mol_path, ico_path, scale_factor=6.2):
    ...
    try:
        ...
        flag = not (
            scale_factor * 0.9 <= ratio <= scale_factor * 1.1
        )  # 10% buffer for e.g., 1+z correction
        return {"sigma_mol_ico_ratio": float(ratio), "flag_sigma_mol_ico": flag}
    except Exception as e:
        return {
            "sigma_mol_ico_ratio": np.nan,
            "flag_sigma_mol_ico": True,
            "sigma_mol_ico_error": str(e),
        }
...
def compare_lco_to_ico(lco_path, ico_path, pixel_area_pc2=1.0):
    ...
    try:
        ...
        with fits.open(ico_path) as hdul:
            ico = hdul[0].data
        ratio = (
            np.nanmean(lco) / (np.nanmean(ico) * pixel_area_pc2)
            if np.nanmean(ico) != 0
            else np.nan
        )
...
def compare_mmol_to_lco(mmol_path, lco_path):
    ...
    try:
        ...
        ratio = (
            np.nanmean(mmol) / (6.2 * np.nanmean(lco))
            if np.nanmean(lco) != 0
            else np.nan
        )
        flag = not (0.9 <= ratio <= 1.1)  # allow 10% tolerance
        ...
        return {
            "mmol_lco_ratio": np.nan,
            "flag_mmol_lco": True,
            "mmol_lco_error": str(e),
        }
...
def check_map_detection(map_path, threshold_sigma=3):
    ...
    with fits.open(map_path) as hdul:
        ...
        return {
            "map_detected": detected,
            "map_max": float(max_val),
            "map_rms": float(rms),
        }
...
def check_lco_larger_than_ico(lco_path, ico_path):
    ...
    return {
        "lco_sum": float(lco_sum),
        "ico_sum": float(ico_sum),
        "lco_mean": float(lco_mean),
        "ico_mean": float(ico_mean),
        "flag_lco_gt_ico": flag,
    }


def check_scaling_factor_consistency(
    map1_path, map2_path, err1_path=None, err2_path=None
):
    ...
    if (
        err1_path
        and err2_path
        and os.path.exists(err1_path)
        and os.path.exists(err2_path)
    ):
        ...
        with fits.open(err2_path) as hdul:
            err2 = hdul[0].data
        err_scale = (
            np.nanmean(err2) / np.nanmean(err1) if np.nanmean(err1) != 0 else np.nan
        )
        result["error_scaling_factor"] = float(err_scale)
        result["flag_scaling_consistency"] = not np.isclose(
            scale, err_scale, rtol=0.05
        )  # 5% tolerance
    else:
            ...
            result.update(
                {
                    ...
                    "fail_unit": fail,
                    "fail_reason": reason,
                    "fits_compliant": fits_compliant,
                    "fits_compliance_error": fits_compliance_error,
                }
            )
    except Exception as e:
...
def run_wcs_validation(fits_path):
    ...
    try:
        ...
        with fits.open(fits_path) as hdul:
            ...
            if missing:
                return {
                    "wcs_validation": str(results),
                    "wcs_validation_fail": True,
                    "missing_wcs_keywords": missing,
                }
            ...
            if err is not None and data.shape == snr.shape and err.shape == snr.shape:
                ...
                if np.any(valid):
                    ...
                    if result[f"flag_{prefix}_snr_mismatch"]:
                        result[f"{prefix}_snr_mismatch_summary"] = (
                            f"{frac_mismatch:.2%} of pixels differ by >1%"
                        )
                else:
                ...
                result[f"{prefix}_snr_shape_mismatch"] = (
                    f"map shape {data.shape}, err shape {None if err is None else err.shape}, snr shape {snr.shape if 'snr' in locals() else None}"
                )
    except Exception as e:
